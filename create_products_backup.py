...
def main():
    master_id = 30000
    # open product file and get number of products
    product_dict = csv.DictReader(codecs.open('product-catalog.csv', 'r', 'utf-8'), delimiter='|', quotechar = "'") 
    product_import_file = codecs.open('woocommerce-import.csv', 'w', 'utf-8')
...
def write_product_master(product, id, output_file):
    ...
    image_list = [media_url + product['id'] + '-PV-' + str(i).zfill(2) + '.jpg' for i in range(1, len(product['styles'].split(";")) + 1)]
    image_array = ''
    for i in image_list:
    ...
    # generate quoted string with comma-separated array from style_list
    style_array = ''
    for s in style_list:
        style_array = style_array + s + ', '
    ...
    product_master = {
        ...
        'attribut_2_werte': '"' + product['styles'].replace(";", ",") + '"',
        'attribut_2_sichtbar': '1',
        'attribut_2_global': '0',
        'attribut_2_standard': '"' + style_list[1] + '"',
        'meta_unit': '',
        'meta_unit_base': '',
        'meta_unit_product': '',
...
def write_product_variation(product, id, output_file):
    ...
    j = 0
    for s in style_list:
        if (style_list[j] in product['styles'].split(";")):
            index =  (product['styles'].split(";")).index(style_list[j])
            for df in din_formats:
                ...
                product_variation = {
                    'id': str(id + i),
                    'typ': 'variation',
                    'artikelnummer': str(product['id']).rstrip() + '-' + df + '-' + str(j + 1).zfill(2),
                    'name': product['name'] + '-' + df + '-' + style_list[j],
                    'veröffentlicht': '1',
                    'ist_hervorgehoben': '0',
                    'sichtbarkeit_im_katalog': 'visible',
                    'kurzbeschreibung': '',
                    ...
                    'schlagwörter': '',
                    'versandklasse': '',
                    'bilder': media_url + product['id'] + '-PV-0' + str(j + 1) + '.jpg',
                    'downloadlimit': '',
                    'ablauftage_des_downloads': '',
                    ...
                    'grundpreiseinheit': '',
                    'produkteinheit': '',
                    'warenkorbkurzbeschreibung': product['name'] + '-' + df + '-' + style_list[j],
                    'lieferzeit': '',
                    ...
                    'attribut_1_global': '0',
                    'attribut_1_standard': '',
                    'attribut_2_name': 'Hintergrund',
                    'attribut_2_werte': style_list[j],
                    'attribut_2_sichtbar': '',
                    'attribut_2_global': '0',
                    'attribut_2_standard': style_list[1],
                    'meta_unit': '',
                    ...
                    'meta_mini_desc': str(product['id']).rstrip() + '-' + df + '-' + str(j + 1).zfill(2),
                    'meta_min_age': '',
                    'meta_free_shipping': '',
                    'meta_service': 'no',
